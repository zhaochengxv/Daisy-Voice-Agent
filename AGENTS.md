...
- **Windows 支持**：核心循环（录音→ASR→LLM→TTS→UI）单实现跨平台；系统控制层按平台分派——Windows 用 PowerShell/user32/Outlook/Word COM 等价实现（开关应用、音量、播放控制、锁屏、剪贴板、计时器/闹钟、分屏、勿扰、备忘录、邮件、日历、文档读写转换、音频输出切换等），唯一降级为 `edit_pdf`（Windows 无免费内置 PDF 编辑方案）。
